...
上面的配置使得``RegistryDirectory``对象作为监听器监听服务端在注册中心的``providers,configurators,routers``节点的配置变化，当配置发生变化时，``RegistryDirectory``对象``notify``方法会被调用，方法参数是注册中心中最新的providers或configurators或routers节点下的url列表，每个列表就代表一个服务端配置，该方法代码：
[java]
----
@Override
...
``refreshOverrideAndInvoker`方法通过``toInvokers``方法将url转换为invoker，该方法代码如下：
[java]
----
